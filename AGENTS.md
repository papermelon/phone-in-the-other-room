...
- Handle build-number selection and project regeneration as part of TestFlight preparation; do not leave the founder to edit the number or ask separately. A number-only request authorizes reserving the next number with the existing locked allocator and regenerating the project, without creating or uploading an archive. Use the complete archive workflow when an archive is requested.
...
- Cumulative Farm credit survives early endings and excludes Brief Access. Wind Down includes eligible overnight timer time up to actual/planned end and excludes the separately rewarded morning window. Its seven-hour search trail also accepts the version-2 bedtime bonus: 20 percentage points once per anchored night, separately accounted and never converted into factual minutes or wool growth. Phone Away has its own 100-minute meter with no three-night gate. Both modes' eligible timer credit grows wool. Factual before-bed minutes, completion counts, Health, and social metrics remain separate; preserve settled rewards and idempotent supported-history time backfill, with no historical bedtime bonuses (ADR-0020).
