"""Recreate ONLY pasture_release on the disposable local PostgreSQL socket/port.
...
e=Path('docs/evidence/pasture-redesign-20260912')
source=(e/'isolated-bootstrap.sql').read_text()
...
subprocess.run([str(pg/'dropdb'),*base,'pasture_release'],check=True)
subprocess.run([str(pg/'createdb'),*base,'pasture_release'],check=True)
p=[str(pg/'psql'),*base,'-d','pasture_release','-v','ON_ERROR_STOP=1']
...
subprocess.run([*p,'-f',str(e/'legacy-auth-fixture-adapter.sql')],capture_output=True,check=True)
...
suites=['shared_pasture_test.sql','night_flock_v4_test.sql','night_flock_membership_sharing_test.sql','night_flock_v4_social_avatar_test.sql','slumber_party_farm_cheer_receipts_test.sql','night_flock_shared_habits_test.sql','farm_save_test.sql','account_identity_test.sql']
results=[]
for suite in suites:
...
with (e/'concurrent-placements.log').open('w') as f:
 subprocess.run(['python3','supabase/tests/shared_pasture_concurrency.py'],stdout=f,stderr=subprocess.STDOUT,check=True)
print('concurrent placements/grants passed',flush=True)
