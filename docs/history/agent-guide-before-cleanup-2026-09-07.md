# Historical agent guide — before 7 September 2026 cleanup

This is a verbatim preservation of the pre-cleanup working-tree guide, including superseded instructions. It is evidence, not operative guidance. Use [the current guide](../../AGENTS.md). Read this snapshot only for historical comparison.
...
## Account-owned Farms and password sign-in — 7 September 2026

The founder selected account-owned Farms with automatic private synchronization as part
of account use, replacing separate optional backup enrollment. Signing in loads that
account's Farm; signing out removes it from the active, playable experience, and a different
account never inherits it. Apple sign-in remains, with username/handle or email plus password
also required. See ADR-0023 and `docs/plans/account-owned-farm-sync.md`. The founder authorized implementation. Local source now includes owner-scoped Farm activation,
automatic sync, compact Account screens and password authentication. Production activation and
physical-device validation remain separate gates; see the plan’s implementation evidence. The existing private Farm payload
boundary and truthful confirmed-save claims remain. Earlier backup and onboarding sections
below describe the prior implementation where superseded by this direction.

## Onboarding account experience — 6 September 2026

The founder approved a single returning-user sign-in entrance on the first welcome page,
and a dedicated optional account invitation after the Shepherd/welcome-gift stage and before
Wind Down configuration. Native Apple sign-in, explicit backup consent, and confirmed online
saving remain distinct. A successful, explicitly chosen Farm restore routes returning users
to device setup without repeating the questionnaire or gift journey. Authentication alone
never means a Farm was restored. Local continuation and welcome-gift eligibility remain
available without an account. Repeated story/ready-screen account prompts are superseded;
the final first-run action is **Go to Home**. See
`docs/plans/onboarding-account-experience.md` and ADR-0022. Implementation and acceptance
are tracked in the plan; approval does not imply a distributed build.

## Account-backed Farm save — 5 September 2026

The founder approved the plan in `docs/plans/account-backed-farm-save.md` and
authorized implementation. ADR-0021 records the local persistence exception:
Farm/progress/search/reward ledgers and the device-only settlement journal use
one versioned JSON transaction store in Application Support, with recovery copies.
Other app settings/history remain in UserDefaults. Do not serialize this local
document as a cloud payload: the private Farm field boundary is specified in
`docs/plans/farm-save-contract.md`. The founder explicitly authorized production Supabase deployment, verified Apple
sign-in configuration, updated backup disclosures, and physical-device testing on
5 September 2026. Implementation and validation are in progress; approval alone does
not mean a Farm is backed up online. Only a confirmed server save supports that claim.

## Cumulative Farm credit — 5 September 2026

The founder now requires cumulative Farm credit that survives early endings and
excludes Brief Access. [ADR-0020](docs/DECISIONS/ADR-0020-cumulative-farm-credit.md)
...
- Honest about what it measures: Wind Down before-bed minutes, Screen-Free Morning eligible
  elapsed minutes, Phone Away elapsed minutes, and separately classified observed shield evidence
...
5. New Wind Down, Screen-Free Morning, and Phone Away starts require Family Controls authorization
   ...
   Permission and a saved opaque selection are readiness, not proof that a shield ran. Receipts
   distinguish requested protection from observed apply/clear callbacks, partial observation,
   unavailable observation, and sessions where protection was not requested. Brief Access lifts
   the selected-app/category limit temporarily while the session timer continues; Screen-Free
   Morning therefore reports eligible elapsed time, not verified no-screen time.
...
7. ADR-0016's v4 Slumber Party source implementation is complete locally. It presents a list of up to
   ...
   has no in-app social UI. Current members may see app-recorded, self-reported round records, revisioned expiring
...
The iPhone is the **authoritative** side of a run. The Watch mirrors the phone-authoritative timer
and state; current release flows do not use it as physical-placement evidence.
...
- `HomeView` is the navigation shell: during an active Wind Down Home becomes the live journey while Nights, Farm, and Settings remain reachable; terminal receipts temporarily override the shell. New screens hook into that routing, not parallel navigation stacks.
...
- “Put the phone in another room” is the primary promise, invitation, and ritual action. Timer
  completion, shielding authorization, NFC confirmation, Watch state, and app history do not prove
  continuous physical placement, sleep, complete screen avoidance, or routine completion.
...
- **Expanding shielding beyond its consented boundary.** Reporting/pickers and the required
  start-readiness barrier are enabled; shielding must use the consented selection, run only from an
...
| Selected-app barrier + optional NFC start | Included for 1.0; barrier readiness required, NFC optional | New extension App IDs, Family Controls distribution, and physical overnight QA |
...
2. Privacy strings consistent with Wind Down and Screen-Free Morning; App Store privacy labels cover the optional
...
4. App Store 1.0 stays four tabs (Home + Nights + Farm + Settings). Farm may contain The Barn,
   ...
   Debug preview flag. NFC is optional; read-only HealthKit sleep is optional context; the
   selected-app/category barrier is required readiness for new starts. Runtime shield evidence
   remains distinct from authorization and from private Screen Time reports.
