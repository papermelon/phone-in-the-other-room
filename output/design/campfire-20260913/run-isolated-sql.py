"""Recreate ONLY campfire_release on the disposable local PostgreSQL socket/port.
...
e=Path('output/design/campfire-20260913')
...
subprocess.run([str(pg/'dropdb'),*base,'--if-exists','campfire_release'],check=True)
subprocess.run([str(pg/'createdb'),*base,'campfire_release'],check=True)
p=[str(pg/'psql'),*base,'-d','campfire_release','-v','ON_ERROR_STOP=1']
...
suites=['campfire_test.sql','shared_pasture_test.sql','night_flock_v4_test.sql','night_flock_membership_sharing_test.sql','night_flock_v4_social_avatar_test.sql','slumber_party_farm_cheer_receipts_test.sql','night_flock_shared_habits_test.sql','farm_save_test.sql','account_identity_test.sql']
results=[]
for suite in suites:
